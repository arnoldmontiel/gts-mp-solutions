...
public abstract class TrackMap extends WebPageAdaptor implements Constants {
	...
	private static final long DFT_AUTO_MAXCOUNT = DFT_AUTO_DURATION
			/ DFT_AUTO_INTERVAL;
	...
	private static final String MAP_UPDATE_LAST[] = new String[] { "last",
			"false" };

	// PrivateLabel.PROP_TrackMap_autoUpdateRecenter
	private static final String AUTO_RECENTER_NONE[] = new String[] { "no",
			"0", "false", "none" };
	private static final String AUTO_RECENTER_LAST[] = new String[] { "last",
			"1" };
	private static final String AUTO_RECENTER_ZOOM[] = new String[] { "zoom",
			"2", "true", "yes" };
	private static final String AUTO_RECENTER_PAN[] = new String[] { "pan", "3" };

	// PrivateLabel.PROP_TrackMap_showLocateNow
	private static final String SHOW_PING_FALSE[] = new String[] { "false",
			"no" };
	...
	private static final String CALENDAR_DATE_NOW[] = new String[] { "current",
			"now" };
	private static final String CALENDAR_DATE_LAST[] = new String[] { "last",
			"device" };

	// PrivateLabel.PROP_TrackMap_mapControlLocation
	private static final String CONTROLS_ON_LEFT[] = new String[] { "left",
			"true" };
	...
	protected void postInit() {
		...
		/* status codes */
		this.statusCodes = this.getProperties().getIntArray(PROP_statusCodes,
				null);
		/**
		 * / if (!ListTools.isEmpty(this.statusCodes)) { for (int i = 0; i <
		 * this.statusCodes.length; i++) { Print.logInfo("Map StatusCode: 0x" +
		 * StringTools.toHexString(this.statusCodes[i],16)); } } /
		 **/

		/* Fleet: showFleetFromCalendar */
		if (this.isFleet()) {
			// fleet map
			String frCal = this.getProperties().getString(
					PROP_showFleetFromCalendar, "");
			this.showFromCalendar = (StringTools.isBlank(frCal) || frCal
					.equalsIgnoreCase("default")) ? false : StringTools
					.parseBoolean(frCal, false);
		} else {
			// device map
	...
	protected void writeJS_MapUpdate(final RequestProperties reqState,
			PrintWriter out, String mapUpdURL, String devicePingURL,
			String kmlUpdURL, boolean autoUpdateEnabled,
			boolean autoUpdateOnLoad, long autoInterval, long autoMaxCount,
			boolean mapControlsOnLeft, int showBatteryLevel) throws IOException {
		...
		switch (Calendar.getCalendarAction(this.getStringProperty(privLabel,
				PrivateLabel.PROP_TrackMap_calendarAction, null))) {
		case FIXED:
			calCollapse = false;
			calFade = false;
			calDivBox = false;
		...
		/* Calendar OnLoad */
		String calDateOnLoad = this.getStringProperty(privLabel,
				PrivateLabel.PROP_TrackMap_calendarDateOnLoad,
				CALENDAR_DATE_NOW[0]).toLowerCase();
		...
		if (isFleet) {
			...
			String muol = this.getStringProperty(privLabel,
					PrivateLabel.PROP_TrackMap_mapUpdateOnLoad, "");
			mapUpdateOnLoad = ListTools.containsIgnoreCase(MAP_UPDATE_LAST,
					muol) ? MAP_UPDATE_LAST[0] : MAP_UPDATE_ALL[0];
		}

		/* auto-update attributes */
		int autoUpdateRecenterMode = 0;
		if (autoUpdateEnabled) {
			String mode = this.getStringProperty(privLabel,
					PrivateLabel.PROP_TrackMap_autoUpdateRecenter,
					AUTO_RECENTER_ZOOM[0]);
			if (ListTools.containsIgnoreCase(AUTO_RECENTER_NONE, mode)) {
				autoUpdateRecenterMode = 0; // none
			} else if (ListTools.containsIgnoreCase(AUTO_RECENTER_LAST, mode)) {
				autoUpdateRecenterMode = 1; // last
			} else if (ListTools.containsIgnoreCase(AUTO_RECENTER_PAN, mode)) {
		...
		JavaScriptTools.writeJSVar(out, "AutoUpdateRecenterMode",
		...
		JavaScriptTools.writeJSVar(out, "LimitType", this.getStringProperty(
				privLabel, PrivateLabel.PROP_TrackMap_limitType, "last"));
		JavaScriptTools.writeJSVar(out, "ID_MAP_UPDATE_BTN", ID_MAP_UPDATE_BTN);
		JavaScriptTools.writeJSVar(out, "ID_MAP_AUTOUPDATE_BTN",
				ID_MAP_AUTOUPDATE_BTN);
		...
		JavaScriptTools.writeJSVar(out, "ID_MAP_CONTROL_BAR",
				ID_MAP_CONTROL_BAR);
		if (mapControlsOnLeft) {
			JavaScriptTools.writeJSVar(out, "CLASS_CONTROL_BAR", new String[] {
					"mapControlCollapseBar_L", "mapControlCollapseBar_R" });
		} else {
			JavaScriptTools.writeJSVar(out, "CLASS_CONTROL_BAR", new String[] {
					"mapControlCollapseBar_R", "mapControlCollapseBar_L" });
		}

		/* Localized text */
		out.write("// TrackMap localized text\n");
		JavaScriptTools.writeJSVar(out, "TEXT_autoUpdateStart",
				i18n.getString("TrackMap.startAutoUpdate", "Auto"));
		JavaScriptTools.writeJSVar(out, "TEXT_autoUpdateStop",
				i18n.getString("TrackMap.stopAutoUpdate", "Stop"));
		...
		JavaScriptTools.writeJSVar(out, "PARM_RANGE_FR",
				Calendar.PARM_RANGE_FR[0]);
		JavaScriptTools.writeJSVar(out, "PARM_RANGE_TO",
				Calendar.PARM_RANGE_TO[0]);
		JavaScriptTools.writeJSVar(out, "PARM_TIMEZONE",
				Calendar.PARM_TIMEZONE[0]);
		JavaScriptTools.writeJSVar(out, "PARM_LIMIT", PARM_MAP_LIMIT);
		JavaScriptTools.writeJSVar(out, "PARM_ALARM", "alarm");
		...
		JavaScriptTools.writeJSVar(out, "PARM_DEVICE_COMMAND",
				PARM_DEVICE_COMMAND);
		...
		if (!ListTools.isEmpty(mapShapes)) {
			...
			for (Iterator<MapShape> msi = mapShapes.values().iterator(); msi
					.hasNext();) {
		...
		/* device links */
		/*
		 * if (isFleet) { // TODO: } else { Device device =
		 * reqState.getSelectedDevice(); if (device != null) {
		 * JavaScriptTools.writeJSVar(out, "DeviceLinkURL" ,
		 * device.getLinkURL()); JavaScriptTools.writeJSVar(out,
		 * "DeviceLinkDescription" , device.getLinkDescription()); } else {
		...
		if (DeviceChooser.isDeviceChooserUseTable(privLabel)) {
			// DeviceChooser.writeDeviceList(out, reqState,
			// "TrackSelectorList");
		}
		...
		if (this.showFromCalendar) {
			...
			Calendar.writeNewCalendar(out, CALENDAR_FROM, null,
					i18n.getString("TrackMap.dateFrom", "From"),
					reqState.getEventDateFrom());
		} else {
			JavaScriptTools.writeJSVar(out, CALENDAR_FROM, null);
		}
		Calendar.writeNewCalendar(out, CALENDAR_TO, null,
				i18n.getString("TrackMap.dateTo", "To"),
				reqState.getEventDateTo());
		...
		JavaScriptTools.writeJSInclude(out,
				JavaScriptTools.qualifyJSFileRef("TrackMap.js"));

		/* sorttable.js */
		if (DeviceChooser.isDeviceChooserUseTable(privLabel)) {
			JavaScriptTools.writeJSInclude(out, JavaScriptTools
					.qualifyJSFileRef(ReportPresentation.SORTTABLE_JS));
		}
	...
	// TODO: move this into WebPageAdapter
	private String getStringProperty(PrivateLabel privLabel, String key,
			String dft) {
        ...
        throws IOException
    {
        ...
        String  alarm    = (String)AttributeTools.getRequestAttribute(request, "PARM_ALARM", "");
        ...
        /* no defined Meitrack? */
        final Meitrack meitrack;
        Meitrack mei = null;
        if (!isFleet) {
            
        	String devID = device.getDeviceID();
        	try {
        		mei = Meitrack.getMeitrack(currAcct, devID);	
        ...
        if (SubmitMatch(submitEdit,"agregoState")) {
        	//device.setStateRequest(1);
        	//Device.getFactory().hasField("stateRequest");
        	//device.setRemotePortCurrent(1);
        	int input1 = meitrack.getInput1();
        	if(input1 == 1)
        		meitrack.setInput1(0);
        	else
        		meitrack.setInput1(1);
        ...
        HTMLOutput HTML_CONTENT = new HTMLOutput((mapAutoSize? CommonServlet.CSS_CONTENT_MAP_FULL : CommonServlet.CSS_CONTENT_MAP), m) {
            public void write(PrintWriter out) throws IOException {
                ...
                out.println("\n<!-- Command form -->");
                if(meitrack != null)
                	out.println("input1: " + meitrack.getInput1() + " account: " +meitrack.getAccountID() );
                
                
          
                out.println("la nombre: " + reqState.getPageName() );
                ...
                if (TrackMap.this.getBooleanProperty(privLabel,PrivateLabel.PROP_TrackMap_enableGeocode,false)) {
                    ...
                    if (gcp.getName().startsWith("geonames")) {
                        ...
                        out.println("<form id='"+FORM_GOTO_ADDRESS+"' name='"+FORM_GOTO_ADDRESS+"' method='get' action=\"javascript:true;\" target='_self'>"); // target='_top'
                ...
                if (showTimezoneSelect) {
                    out.println("\n<!-- Timezone select -->");
                    out.println("<form id='"+FORM_SELECT_TIMEZONE+"' name='"+FORM_SELECT_TIMEZONE+"' method='get' action=\"javascript:true;\" target='_self'>"); // target='_top'
                ...
                */
                ...
                //out.print("<form id='jaja' name='jojo' method='get' action='javascript:true;' target='_self'>"); // target='_top'
                out.print("<form name='UserInfoSelect' method='post' action='./Track?page=map.device' target='_self'>");
                ...
                out.print("<td id='botonloco'>");
                out.write("<input type='submit' name='"+PARM_SUBMIT_EDIT+"' value='agregoState'>");
                out.print("</td>");
                out.print("</tr></table>\n");
                out.print("</form>");
