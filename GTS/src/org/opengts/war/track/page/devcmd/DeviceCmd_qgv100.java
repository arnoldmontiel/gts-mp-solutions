    ...
    implements DeviceCmdHandler
{
    ...
    /* device code */
    public static final String  DEVICE_CODE                 = DCServerFactory.QGV100_NAME;
        ...
        throws IOException
    {
        ...
        I18N         i18n        = privLabel.getI18N(DeviceCmd_qgv100.class);
        ...
        out.println(DeviceInfo.FormRow_TextField(PARM_DEVICE  , false, i18n.getString("DeviceCmd_qgv100.deviceID","{0} ID",devTitles)+":"            , selDevID, 30, 30));
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_DESC, false, i18n.getString("DeviceCmd_qgv100.deviceDesc","{0} Description",devTitles) +":", (selDev!=null)?selDev.getDescription():"", 40, 40));
        ...
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_LAST_CONNECT, false, i18n.getString("DeviceCmd_qgv100.lastCommunication","Last Communication") +":", lastCommTime , 30, 30));
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_LAST_EVENT  , false, i18n.getString("DeviceCmd_qgv100.lastEvent","Last Event") +":"                , lastEventTime, 30, 30));

        /* commands */
        out.println("<tr class='deviceCommandList'>");
        out.println("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_HEADER+"' nowrap style=\"vertical-align:top; padding-top:5px;\">"+i18n.getString("DeviceCmd_qgv100.commandSelect","Command Select")+":</td>");
        ...
        if (dcs != null) {
            ...
            if (!ListTools.isEmpty(cmdMap_admin)) {
                cmdCount += this._writeCommands(out, reqState, 
                    i18n.getString("DeviceCmd_qgv100.standardCommands","Standard Commands"),
            ...
            if (!ListTools.isEmpty(cmdMap_sysadmin)) {
                cmdCount += this._writeCommands(out, reqState, 
                    i18n.getString("DeviceCmd_qgv100.sysadminCommands","SysAdmin Commands"),
        ...
        if (cmdCount == 0) {
            ...
            out.print("<span>"+i18n.getString("DeviceCmd_qgv100.noCommandsAvailable","No Commands Available")+"</span>");
            out.print("</td>");
            out.print("</tr>\n");
        }
        ...
        out.println(DeviceInfo.FormRow_ComboBox(PARM_COMMAND_SELECT , editProps, i18n.getString("DeviceCmd_qgv100.commands","Command")+":", commandSel, commandMap, "", -1));
        */
        ...
        if (editProps) {
            out.write("<input type='submit' name='"+PARM_SUBMIT_SEND+"' value='"+i18n.getString("DeviceCmd_qgv100.send","Send")+"'>\n");
            out.write("<span style='padding-left:10px'>&nbsp;</span>\n");
            out.write("<input type='button' name='"+PARM_BUTTON_CANCEL+"' value='"+i18n.getString("DeviceCmd_qgv100.cancel","Cancel")+"' onclick=\"javascript:openURL('"+actionURL+"','_self');\">\n");
        } else {
            out.write("<input type='button' name='"+PARM_BUTTON_BACK+"' value='"+i18n.getString("DeviceCmd_qgv100.back","Back")+"' onclick=\"javascript:openURL('"+actionURL+"','_self');\">\n");
        }
        out.write("</form>\n");
        return true;
    ...
    {
        if (resp == null) {
            return i18n.getString("DeviceCmd_qgv100.unableToQueueCommand","Unable to queue command for transmission");
        } else {
            // TODO: check response for other possible errors
            Print.logInfo("Response: " + resp);
            return i18n.getString("DeviceCmd_qgv100.commandQueued","Requested command has been queued for transmission");
        }
    }
    ...
    public String handleDeviceCommands(RequestProperties reqState, Device selDev)
    {
        ...
        /* supported device? */
        if (!this.deviceSupportsCommands(selDev)) {
            return i18n.getString("DeviceCmd_qgv100.doesNotSupport","Device does not support {0}", this.getServerDescription());
        }

        /* get selected command */
        String cmdSel = AttributeTools.getRequestString(request, PARM_COMMAND_SELECT, "");
        if (StringTools.isBlank(cmdSel)) {
            return i18n.getString("DeviceCmd_qgv100.noCommandSelected","No command selected");
        }
        ...
        */
        ...
        /* unknown command */
        return i18n.getString("DeviceCmd_qgv100.unknownCommand","No command selected");
