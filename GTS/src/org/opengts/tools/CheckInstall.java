...
public class CheckInstall
{
    ...
    private static final String PROP_SKIP_DFT_EMAIL_CHECKS  = "checkInstall.skipDefaultEMailChecks";
    private static final String PROP_SKIP_DFT_MAP_CHECKS    = "checkInstall.skipDefaultMapChecks";
        ...
        if (defaultConfigFile != null) {
            ...
            if (driver == null) {
                ...
                for (Iterator i = factMap.keyIterator(); i.hasNext();) {
                    ...
                    try {
                        if (!f.tableExists()) {
                            printVariable(f.getTableName(), "", "(ERROR: table does not exist!)");
                            addError("Table '"+f.getTableName()+"' does not exist.",
                                     "Database may not have been initialized.",
                                     "Please initialize the database.");
                        } else
                        if (!f.validateColumns(0x0000)) {
                            printVariable(f.getTableName(), "", "(ERROR: column validation failed!)");
                            addError("Table '"+f.getTableName()+"' failed column validation.",
                            ...
                            printVariable(f.getTableName(), "RecordCount "+rcdCnt, "");
                        }
                        ...
                        if (dbe.isSQLException()) {
                            ...
                            if (sqlMsg.indexOf("no suitable driver") >= 0) {
                                ...
                                addError("SQL database exception while checking table '"+f.getTableName()+"' existance.",
                            ...
                            addError("Database exception while checking table '"+f.getTableName()+"' existance.",
                                     "Refer to above stacktrace for a detailed description",
                                     null);
                            dbe.printException();
                        }
        ...
        println("private.xml:");
        {
            ...
            if (BasicPrivateLabelLoader.hasParsingErrors()) {
                ...
                for (BasicPrivateLabel privLabel : privLabelSet) {
                    ...
                    boolean skipDefaultEMailChecks = isDefault && RTConfig.getBoolean(PROP_SKIP_DFT_EMAIL_CHECKS,false);
                    boolean skipDefaultMapChecks   = isDefault && RTConfig.getBoolean(PROP_SKIP_DFT_MAP_CHECKS,false);
