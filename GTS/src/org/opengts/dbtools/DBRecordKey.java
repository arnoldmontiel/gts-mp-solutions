...
public abstract class DBRecordKey<gDBR extends DBRecord>
{
    ...
    **/
    ...
    {
        return this.getFactory().getTranslateTableName();
    }
        ...
        for (int i = 0; i < keyFlds.length; i++) {
            ...
            if (fldVals.hasFieldValue(fldName)) {
                if (!hasPartialKey || (whereKeyType == DBWhere.KEY_PARTIAL_ALL)) {
                    ...
                    String m = "Additional partial key in 'WHERE' clause! [" + this.getTableName() + "." + fldName + "]";
            ...
            if ((i == 0) && (whereKeyType != DBWhere.KEY_PARTIAL_ALL_EMPTY)) { // 
                ...
                if (keyFlds[i].isAutoIncrement()) {
                    ...
                    String m = "First key field for 'WHERE' clause is 'auto_increment' and field is not present [" + this.getTableName() + "." + fldName + "]";
                    throw new DBException(m);
                } else {
                    String m = "Missing first key field for 'WHERE' clause! [" + this.getTableName() + "." + fldName + "]";
                    throw new DBException(m);
                }
            } else
            if (whereKeyType == DBWhere.KEY_FULL) {
