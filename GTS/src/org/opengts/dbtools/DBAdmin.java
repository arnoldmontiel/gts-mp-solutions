...
public class DBAdmin
{
    ...
    **/
    public static void addTableFactory(DBFactory<? extends DBRecord> factory)
    {
        ...
        if (factory.getFieldCount() <= 0) {
            Print.logStackTrace("Ignoring DBFactory with no fields: " + factory.getTableName());
        } else {
            String tableName = factory.getTableName().toLowerCase();
            if (DBAdmin.getTableFactoryMap().containsKey(tableName)) {
                Print.logWarn("Overriding existing DBFactory: %s", factory.getTableName());
            }
    ...
    **/
    public static DBFactory<? extends DBRecord>[] getChildTableFactories(DBFactory<? extends DBRecord> parentFactory)
    {
        ...
        if (parentFactory != null) {
            String parentTable = parentFactory.getTableName();
            for (DBFactory<? extends DBRecord> child : DBAdmin.getTableFactoryMap().values()) {
                String childTable = child.getTableName();
                if (child.hasParentTable(parentTable)) {
    ...
    **/
    private static DBFactory<? extends DBRecord> _getTableFactory(String tableName)
    {
        if (tableName != null) {
            String tn = tableName.toLowerCase();
            return (DBFactory<? extends DBRecord>)DBAdmin.getTableFactoryMap().get(tn);
        } else {
            return null;
        }
    }
    ...
    *** @return The table factory
    **/
    public static DBFactory<? extends DBRecord> getTableFactory(String tableName)
    {
        if (StringTools.isBlank(tableName)) {
            Print.logWarn("Table name is null/blank");
            return null;
        } else {
            DBFactory<? extends DBRecord> fact = DBAdmin._getTableFactory(tableName);
            if (fact == null) {
                Print.logWarn("Table factory class not found: " + tableName);
                Print.logStackTrace("Table factory class not found: " + tableName);
                return null;
            } else {
                return fact;
            }
    ...
    **/
    public static boolean hasTableFactory(String tableName)
    {
        return (DBAdmin.getTableFactory(tableName) != null);
    }
    ...
    **/
    public static String getTableNameProperCase(String tableName)
    {
        DBFactory fact = getTableFactory(tableName);
        return (fact != null)? fact.getTableName() : tableName;
    }
    ...
    public static final int VALIDATE_CREATE_TABLE           = 0x0001;   // 't'
    public static final int VALIDATE_ADD_COLUMNS            = 0x0002;   // 'c'
    public static final int VALIDATE_ALTER_COLUMNS          = 0x0004;   // 'a'
    public static final int VALIDATE_REBUILD_KEYS           = 0x0008;   // 'k'
    public static final int VALIDATE_CHECK_ENCODING         = 0x0010;   // 'u' (requires 'a' or 'w')
    public static final int VALIDATE_DISPLAY_ERRORS         = 0x0100;   // 'e'
    public static final int VALIDATE_DISPLAY_WARNINGS       = 0x0200;   // 'w'
    public static final int VALIDATE_SHOW_COLUMNS           = 0x0400;   // 's'
    ...
    *** @param tableName     The table name on which the validation is to be performed
    *** @return True if the validation was performed successfully, false otherwise
    **/
    private static boolean validateTableColumns(final int validateMask, String tableName)
    {
        ...
        for (Iterator<String> i = factMap.keyIterator(); i.hasNext();) {
            ...
            /* specific table */
            if (!StringTools.isBlank(tableName) && !tableName.equalsIgnoreCase(tn)) {
            ...
            try {
                ...
                sb.append("  Table ");
                sb.append(StringTools.leftAlign("'" + f.getTableName() + "'", 25));
                if (!f.tableExists()) {
                ...
                Print.sysPrintln("    ERROR: Unable to create/validate table '" + f.getTableName() + "'");
                dbe.printException();
            }
    ...
    **/
    ...
    {
        // 'tableName' is non-null if the caller wants to validate a specific table
        ...
        for (Iterator<String> i = factMap.keyIterator(); i.hasNext();) {
            ...
            DBFactory<? extends DBRecord> f = (DBFactory<? extends DBRecord>)factMap.get(tn);
            try {
                if (StringTools.isBlank(tableName) || tn.equalsIgnoreCase(tableName)) {
                    if (!f.tableExists()) {
    ...
    *** @param tableName  The specific table name for which the schema will be printed.
    ***                   If null, the schema for all tables will be printed.
    **/
    public static void printTableSchema(int docWidth, String header[], String tableName)
    {
        // 'tableName' is non-null if the caller wants to print the schema for a specific table
        ...
        for (Iterator<String> i = factMap.keyIterator(); i.hasNext();) {
            ...
            try {
                if (f.tableExists()) {
                    ...
                    //Print.sysPrintln("Table does not exist '" + f.getTableName() + "'");
                    //continue;
                }
            } catch (DBException dbe) {
                //Print.logError("Unable to print schema for table '" + f.getTableName() + "'");
                //continue;
            }

            /* omit optional tables? */
            boolean isOptional = f.isOptional();
            if (SKIP_OPTIONAL_TABLES && isOptional) {
            ...
            attr.append(" [");
            attr.append(isOptional?"optional":"required");
            ...
            Print.sysPrintln("");
            Print.sysPrintln(StringTools.replicateString("=",docWidth+1));
            Print.sysPrintln("Table: " + f.getTableName() + attr);
        ...
        */
        ...
        if (dumpTable != null) {
            ...
            for (Iterator<String> i = tableList.iterator(); i.hasNext(); ) {
                ...
                File toDir = dumpDir;
                if (FileTools.hasFileSeparator(tableName)) {
                    // IE. "-dump=./Geozone[.csv]"
                ...
                if (p < 0) {
                    // IE. "Geozone"
                    tableName = DBAdmin.getTableNameProperCase(tableName);
                    dumpFile = new File(toDir, tableName + dftExt);
                } else {
                    // IE. "Geozone.csv"
                    String dotExt = tableName.substring(p);
                    tableName = DBAdmin.getTableNameProperCase(tableName.substring(0,p)); 
                    if (dotExt.equals(DBFactory._DUMP_EXT_CSV) || 
                        dotExt.equals(DBFactory._DUMP_EXT_TXT) || 
                        dotExt.equals(DBFactory._DUMP_EXT_XML)   ) {
                        dumpFile = new File(toDir, tableName + dotExt);
                    } else {
                ...
                if (RTConfig.hasProperty(ARG_WHERE)) {
                    ...
                    Print.logInfo("Dumping '" + tableName + "' to file: " + dumpFile + " => " + where);
                } else {
                    where = null; // all 
                    Print.logInfo("Dumping '" + tableName + "' to file: " + dumpFile);
                }
                // DBFactory for table
                DBFactory<? extends DBRecord> fact = DBAdmin.getTableFactory(tableName);
                if (fact != null) {
                    try {
                        DBSelect dbSel = new DBSelect(fact,where); // "unchecked call"
                        fact.dumpTable(dumpFile, dbSel); // "unchecked conversion"
                    } catch (DBException dbe) {
                        Print.logException("Error dumping table: " + tableName, dbe);
                        return DBAdminExec.ERROR;
                    }
                } else {
                    Print.logError("No DBFactory for table: " + tableName);
                    return DBAdminExec.ERROR;
                }
            }
        }
        ...
        if (RTConfig.hasProperty(ARG_TABLES)) {
            ...
            boolean validateOK = DBAdmin.validateTableColumns(mask,null);
            if (!validateOK) {
                return DBAdminExec.ERROR;
            }
        }
