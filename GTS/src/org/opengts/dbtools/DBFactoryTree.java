...
public class DBFactoryTree
{
    ...
    **/
    ...
    {
        return (this.dbFactory != null)? this.dbFactory.getTableName() : "";
    }
    ...
    **/
    public String getName()
    {
        if (this.dbFactory != null) {
            return this.dbFactory.getTableName();
        } else
        if (this.level == 0) {
            return "<root>";
        } else {
    ...
    ***/
        ...
        DBFactory<? extends DBRecord> dbFact, DBFactoryTree parentNode, Set<String> addedTables)
    {
        ...
        String tableName = dbFact.getTableName();
        ...
        /* add this node */
        //Print.logInfo(StringTools.replicateString("  ",level) + dbFact.getTableName());
        DBFactoryTree dbFactNode = new DBFactoryTree(level, parentNode, dbFact);
        parentNode.addChild(dbFactNode);

        /* find dependent children */
        DBFactory<? extends DBRecord> children[] = dbFact.getChildFactories();
        for (int i = 0; i < children.length; i++) {
            int index = children[i].getParentTables().indexOf(tableName);
            if (level == index) {
                DBFactoryTree._traverseDBFactoryTree(level + 1, children[i], dbFactNode, addedTables);
            } else
            if (!addedTables.contains(children[i].getTableName())) {
                Print.logWarn("Skipping table in heiarchy: " + tableName + " ==> " + children[i].getTableName());
            }
        }
