    ...
    **/
    public String getTableName()
    {
        if (this.recordKey != null) {
            return this.recordKey.getTableName();
        } else {
            return "";
        }
    }
                ...
                if (setPrimaryKey || !fld[i].isPrimaryKey()) {
                    try {
                        ...
                        if (errCode == DBFactory.SQLERR_UNKNOWN_COLUMN) {
                            ...
                            Print.logException("Unknown Column: '" + tblName + "." + fld[i].getName() + "'", sqe);
                        } else
                        if (sqe.getMessage().indexOf("Column") >= 0) {
                            // if it says anything about the "Column"
                            if (RTConfig.isDebugMode()) {
                                Print.logException("Column '" + tblName + "." + fld[i].getName() + "'?", sqe);
                            } else {
                                Print.logError("Column '" + tblName + "." + fld[i].getName() + "'? " + sqe);
                            }
                        } else {
                            throw sqe;
                        }
            ...
            for (int i = 0; i < fld.length; i++) {
                ...
                if (fld[i].isPrimaryKey()) {
                    ...
                    if (requireAllFields) {
                        ...
                        Print.logError("Column '" + tblName + "." + name + "' value not specified.");
                    }
                }
            }
        } else {
