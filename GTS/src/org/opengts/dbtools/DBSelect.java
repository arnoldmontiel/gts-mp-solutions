...
public class DBSelect<gDBR extends DBRecord>
{
        ...
        if (ListTools.isEmpty(sf)) {
            ...
            if (fact != null) {
                for (int i = 0; i < sf.length; i++) {
                    if (sf[i] == null) {
                        Print.logError("DBFactory field is null: %s.%s", fact.getTableName(), sf[i]);
                    } else
                    ...
                    if (sf[i].equalsIgnoreCase(DBProvider.FLD_COUNT())) {
                        ...
                        Print.logError("DBFactory field does not exist: %s.%s", fact.getTableName(), sf[i]);
                    }
                }
            }
    ...
    **/
    ...
    /**
    *** Sets the table name for this DBSelect (if not set, the table name of the defined
    *** DBFactory will be used).
    *** @param tableName  The table name
    **/
    public void setTableName(String tableName)
    {
        this.tableName = ((tableName != null) && !tableName.equals(""))? tableName : null;
    }
        ...
        if ((obf == null) || (obf.length == 0)) {
            ...
            if (fact != null) {
                for (int i = 0; i < obf.length; i++) {
                    if (obf[i] == null) {
                        Print.logError("DBFactory field is null: %s.%s", fact.getTableName(), obf[i]);
                    } else
                    if (fact.hasField(obf[i])) {
                        // ok
                    } else {
                        Print.logError("DBFactory field does not exist: %s.%s", fact.getTableName(), obf[i]);
                    }
                }
            }
    ...
    **/
    private String _toString(boolean inclSelect)
    {
        ...
        if (inclSelect) {
            ...
            /* FROM table */
            sb.append(" FROM ").append(this.getTranslateTableName());
