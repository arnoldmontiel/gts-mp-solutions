    ...
    implements DBRecordListener<gDBR>
{
    ...
    **/
    ...
    {
        String tn = (DBFactory.TableNameMap != null)? DBFactory.TableNameMap.get(tableName) : null;
        ...
        if (DBFactory.factoryHandler != null) {
            try {
                DBFactory<T> fact = DBFactory.factoryHandler.createDBFactory(
                    tableName, field, keyType, rcdClass, keyClass, editable, viewable);
                if (fact != null) {
                    return fact;
                }
            } catch (Throwable th) {
                Print.logException("'<DBFactory.CustomFactoryHandler>.createDBFactory' failed for table " + tableName, th);
            }
        }
        return new DBFactory<T>(tableName, field, keyType, rcdClass, keyClass, editable, viewable);
    }
    ...
    **/
        ...
        boolean editable, boolean viewable) 
    {
        ...
        /* table name */
        this.tableName = StringTools.trim(tableName);
        ...
        this.setOptional((rcdClassStr.indexOf(".opt.") >= 0));
        
        /* class module table name */
        int p = rcdClassStr.lastIndexOf(".");
        if (p > 0) {
            String classTableName = rcdClassStr.substring(p+1);
            DBFactory.addActualTableName(classTableName, this.tableName);
        }
        ...
        if (DBFactory.factoryHandler != null) {
            try {
                ...
                Print.logException("'<DBFactory.CustomFactoryHandler>.selectFields' failed for table " + this.tableName, th);
            }
            if (preList == null) {
                Print.logWarn("'<DBFactory.CustomFactoryHandler>.selectFields' returned NULL for table " + this.tableName);
        ...
        for (DBField fld : preList) {
            ...
            if (this.fieldMap.containsKey(fn)) {
                Print.logError("DBField already defined for table: " + this.tableName + "." + fn);
                continue;
            }
    ...
    **/
    ...
    /**
    *** Sets the 'optional' state of this DBFactory
    ...
    **/
    ...
    /**
    *** Returns true if this table is optional.  
    *** (This is currently determined by whether or not this table is packaged in an ".opt." package)
    *** @return True if this table is optional
    **/
    ...
    **/
    public boolean validateColumns(int validateMask, boolean refreshColumns)
    {
        ...
        if ((colDefined == null) || (colDefined.length == 0)) {
            if (displayErrors) {
                Print.logInfo("ERROR - " + tblName + ": No table columns defined!!!");
            }
            return false;
        }
        if (showColumns) {
            Print.logInfo("    Defined columns: " + tblName);
        ...
        try {;
            ...
            if (colTable == null) {
                if (displayErrors) {
                    Print.logInfo("ERROR - " + tblName + ": Reading table columns not supported!");
                }
                return false;
            }
        } catch (DBException dbe) {
            if (displayErrors) {
                Print.logInfo("ERROR - " + tblName + ": Error reading table columns!");
            }
            return false;
        }
        if (showColumns) {
            ...
            if (showActual) {
                Print.logInfo("    Actual columns: " + tblName + "  (as described in the database)");
                for (int i = 0; i < colTable.length; i++) {
        ...
        for (int i = 0; i < colDefined.length; i++) {
            ...
            if (existingField == null) {
                ...
                if (displayErrors) {
                    Print.logInfo("ERROR - " + tblName + ": Column '" + colDefined[i] + "' [" + i + "] not found");
                }
            ...
            if (colDefined[i].isUTF8() && !existingField.isUTF8()) {
                ...
                if (displayErrors) {
                    Print.logInfo("ERROR - " + tblName + ": Column '" + columnName + "' UTF8 mismatch '" + definedCS + "' != '" + actualCS + "'");
                }
            ...
            if (!colDefined[i].isUTF8() && existingField.isUTF8()) {
                ...
                if (checkColumnEncoding) {
                    if (alterColumnTypes) {
                        ...
                        if (displayWarnings) {
                            Print.logInfo("WARNING - " + tblName + ": Column '" + columnName + "' UTF8 mismatch '" + definedCS + "' != '" + actualCS + "'");
                        }
                    }
                }
            }
            ...
            if (!DBProvider.areTypesEquivalent(definedType,actualType)) {
                ...
                if (displayErrors) {
                    Print.logInfo("ERROR - " + tblName + ": Column '" + columnName + "' Type mismatch expected::" + definedType + " != found:" + actualType + "");
                }
                typeMismatchColumns.add(colDefined[i]);
                columnsOK = false;
            }
            ...
            if (existingField.isPrimaryKey() != colDefined[i].isPrimaryKey()) {
                if (displayErrors) {
                    Print.logInfo("ERROR - " + tblName + ": Column '" + columnName + "' Primary key mismatch");
                }
            ...
            if (colDefined[i].isAlternateKey()) {
                if (!existingField.isAlternateKey()) {
                    if (displayErrors) {
                        Print.logInfo("ERROR - " + tblName + ": Column '" + columnName + "' missing Alternate key");
                    }
                ...
                if (!existingField.equalsAlternateIndexes(colDefined[i].getAlternateIndexes())) {
                    if (displayErrors) {
                        Print.logInfo("ERROR - " + tblName + ": Column '" + columnName + "' Alternate key mismatch");
                    }
                    altKeyMismatchColumns.add(colDefined[i]);
                    columnsOK = false;
                }
                ...
                if (existingField.isAlternateKey()) {
                    if (displayErrors) {
                        Print.logInfo("WARN - " + tblName + ": Column '" + columnName + "' extra Alternate key");
                    }
                    //altKeyMismatchColumns.add(colDefined[i]);
                    //columnsOK = false;
                }
            ...
            if (colTableNdx != i) {
                ...
                if (displayWarnings) {
                    ...
                    //Print.logInfo("WARNING - " + tblName + ": Column '" + columnName + "' [" + i + "] found @ " + colTableNdx);
                }
            }
        ...
        if (displayWarnings) {
            for (int i = 0; i < colTable.length; i++) {
                ...
                if (definedField == null) {
                    Print.logInfo("WARNING - " + tblName + ": Actual column '" + colTable[i] + "' not used");
                }
            }
        }
        ...
        if (addMissingColumns && (missingColumns.size() > 0)) {
            try {
                ...
                if (displayErrors) {
                    Print.logException("ERROR - " + tblName + ": Unable to add missing columns!", dbe);
                }
                return false;
            }
        }
        ...
        if (alterColumnTypes && (typeMismatchColumns.size() > 0)) {
            try {
                ...
                if (displayErrors) {
                    Print.logException("ERROR - " + tblName + ": Unable to alter column type!", dbe);
                }
                return false;
            }
        }
        ...
        if (rebuildKeys && (priKeyMismatchColumns.size() > 0)) {
            try {
                ...
                if (displayErrors) {
                    Print.logException("ERROR - " + tblName + ": Unable to rebuild primary key!", dbe);
                }
                return false;
            }
        }
        ...
        if (rebuildKeys && (altKeyMismatchColumns.size() > 0)) {
            try {
                ...
                if (displayErrors) {
                    Print.logException("ERROR - " + tblName + ": Unable to rebuild alternate keys!", dbe);
                }
                return false;
            }
        }
        ...
        if (!columnsOK && displayWarnings) {
            for (int i = 0; i < colTable.length; i++) {
                Print.logInfo("WARNING - " + tblName + ": Found - " + colTable[i]);
            }
        }
        return columnsOK;
    ...
    **/
        ...
        throws DBException 
    {
        ...
        if (ListTools.isEmpty(valMap) && (partialKeyType != DBWhere.KEY_PARTIAL_ALL_EMPTY)) {
            ...
            throw new DBException("Creating Key: No key fields - " + tn);
        }
        ...
        if (partialKeyType == DBWhere.KEY_AUTO_INDEX) {
            ...
            if (autoKey == null) {
                String tn = this.getTableName();
                throw new DBException("Creating Key: auto-index key not found - " + tn);
            }
        ...
        for (int i = 0; i < pk.length; i++) {
            ...
            if (sval == null) { // either not defined, or no value assigned
                if (partialKeyType == DBWhere.KEY_AUTO_INDEX) {
                    ...
                    String tn = this.getTableName();
                    throw new DBException("Creating Key: auto-index not found - " + tn + "." + name);
                } else
                if (partialKeyType == DBWhere.KEY_FULL) {
                    // all keys required
                    String tn = this.getTableName();
                    throw new DBException("Creating Key: field not found - " + tn + "." + name);
                } else
                ...
                if (i == 0) {
                    ...
                    String tn = this.getTableName();
                    throw new DBException("Creating Key: first field not found - " + tn + "." + name);
                } else
    ...
    **/
        ...
        throws DBException 
    {
        ...
        if (rcdKey != null) {
            ...
            Print.logError("Unable to create record: " + this.getTableName());
            return null;
        }
    }
    ...
    **/
        ...
        throws DBException 
    {
        ...
        if (rcdKey != null) {
            ...
            Print.logError("Unable to create key: " + this.getTableName());
            return null;
        }
    }
    ...
    **/
    ...
    /**
    *** Returns the table for for this DBFactory
    ...
    **/
    ...
    {
        return DBProvider.translateTableName(this.getTableName());
    }
    ...
    **/
        ...
        throws SQLException, DBException
    {
        ...
        if (dbp.getID() == DBProvider.DB_MYSQL) {
            ...
            sb.append("ALTER TABLE ").append(this.getTranslateTableName());
            for (;ndx < cols.length; ndx++){
                ...
                if (colMap.containsKey(colName)) {
                    ...
                    Print.logInfo("Changing column: " + this.getTableName() + "." + col.getFieldDefinition());
                } else {
                    // Add a new column:
                    ...
                    Print.logInfo("Adding column: " + this.getTableName() + "." + col.getFieldDefinition());
                }
                colCount++;
            }
        } else
        if (dbp.getID() == DBProvider.DB_SQLSERVER) {
            ...
            sb.append("ALTER TABLE ").append(this.getTableName());
            DBField col = cols[ndx];
            String colName = col.getName();
            if (colMap.containsKey(colName)) {
                // Change an existing column name:
        ...
        /* execute */
        if (sb.length() > 0) {
            Print.logInfo("Exectuting SQL 'ALTER TABLE ...'"); // + sb);
            DBConnection dbc = null;
            try {
                dbc = DBConnection.getDefaultConnection();
        ...
        throws SQLException, DBException
    {
        ...
        if ((colMap != null) && colMap.containsKey(colName)) {
            ...
            if (dbp.getID() == DBProvider.DB_MYSQL) {
                ...
                Print.logInfo("Dropping column: " + tableName + "." + fieldDef);
            } else
            if (dbp.getID() == DBProvider.DB_SQLSERVER) {
                sb.append("ALTER TABLE ").append(tableName);
                sb.append(" DROP COLUMN ").append(colName);
    ...
    **/
        ...
        throws DBException
    {
        ...
        try {
            DBProvider.removePrimaryIndex(this.getTableName());
        } catch (Throwable th) {
    ...
    **/
        ...
        throws DBException
    {
        ...
        try {
            DBTableIndexMap indexMap = DBProvider.getActualTableIndexMap(TN);
            if (indexMap != null) {
                ...
                if (altIndexSet != null) {
                    for (Iterator<String> i = altIndexSet.iterator(); i.hasNext();) {
                        ...
                        try {
                            DBProvider.removeAlternateIndex(TN, indexName);
                            Print.logInfo("Dropped alternate index '"+indexName+"' from table "+TN);
                        } catch (Throwable th) {
        ...
        if ((altIndexes != null) && (altIndexes.length > 0)) {
            ...
            for (int i = 0; i < altIndexes.length; i++) {
                ...
                try {
                    ...
                    DBProvider.createAlternateIndex(TN, altIndexes[i]);
                    Print.logInfo("Created alternate index '"+indexName+"' for table "+TN);
                } catch (SQLException sqe) {
            ...
            try {
                DBProvider.removeAlternateIndex(TN, null);
            } catch (Throwable th) {
                // This exception is expected
                //Print.logWarn("Alternate key does not currently exist for table: " + TN);
                // ignore exception
            }
    ...
    **/
    ...
    {
        try {
            DBProvider.dropTable(this.getTableName());
        } catch (SQLException sqe) {
        ...
        try {
            ...
            if (outFmt == DUMP_FORMAT_XML) {
                ...
                sbData.append("<" + TAG_Records);
                    sbData.append(" " + ATTR_table + "=\"" + this.getTableName() + "\"");
                    sbData.append(">\n");
            } else
            ...
            while (rs.next()) {
                ...
                if (outFmt == DUMP_FORMAT_XML) {
                    ...
                    for (int i = 0; i < fields.length; i++) {
                        ...
                        DBFactory.writeXML_DBField(sbData, 2*indent, fields[i], false, value);
                    }
    ...
    **/
        ...
        throws DBException, SQLException, IOException
    {
        ...
        if (recordExists) {
            ...
            sbSql.append("UPDATE ").append(this.getTableName());
            sbSql.append(" SET ");
            for (int i = 0; i < oldFieldNames.length; i++) {
                String fieldName = oldFieldNames[i];
                DBField field = this.getField(fieldName);
    ...
    **/
    public void addParentTable(String tableName)
    {
        if ((tableName != null) && !this.parentTables.contains(tableName)) {
            this.parentTables.add(tableName);
        }
    }
    ...
    *** @param tableName The parent table name to test
    *** @return True if this table is a dependent of the specified table, false otherwise
    **/
    public boolean hasParentTable(String tableName)
    {
        return this.parentTables.contains(tableName);
    }
    ...
    **/
        ...
        throws DBException
    {
        ...
        /* table name */
        String tableName = XMLTools.getAttribute(node, ATTR_table, null, false);
        if (StringTools.isBlank(tableName)) {
            throw new DBException("Table name is blank");
        }
        
        /* DBFactory */
        DBFactory tableFact = DBFactory.getFactoryByName(tableName);
        if (tableFact == null) {
            // table not found
            throw new DBException("Table name not found: " + tableName);
        }
        return tableFact;
