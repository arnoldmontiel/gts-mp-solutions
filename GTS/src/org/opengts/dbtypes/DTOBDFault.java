    ...
    extends DBFieldType
{
    ...
    public static String GetPropertyString(long fault)
    {
        ...
        if ((fault & TYPE_MASK) == TYPE_OBDII) {
            ...
            sb.append(" ").append(PROP_DTC[0]).append("=").append(dtc);
        } else {
            // unrecognized
        }
        return sb.toString();
    ...
    public static long EncodeFault_J1708(int mid, boolean isSID, int pidSid, int fmi, int count, boolean active)
    {
        ...
        faultCode |= EncodeActive(active);
        faultCode |= EncodeSystem(mid);
        faultCode |= EncodeSPID(spid);
        faultCode |= EncodeFMI(fmi);
        faultCode |= EncodeCount(count);
        return faultCode;
    }
    ...
    public static long EncodeFault_J1939(int spn, int fmi, int count)
    {
        ...
        faultCode |= EncodeActive(true);
        faultCode |= EncodeSystem(spn);
        faultCode |= EncodeFMI(fmi);
        faultCode |= EncodeCount(count);
        return faultCode;
    }
