    ...
    implements DataTransport
{
  ...
  //public static final String FLD_smsGatewayProps       = "smsGatewayProps";       // SMS gateway properties
    ...
    private static DBField FieldInfo[] = {
        ...
        new DBField(FLD_speedLimitKPH        , Double.TYPE         , DBField.TYPE_DOUBLE      , "Max Speed km/h"              , "edit=2 format=#0.0"),
        ...
        new DBField(FLD_simPhoneNumber       , String.class        , DBField.TYPE_STRING(24)  , "SIM Phone Number"            , Device._simPhoneNumber_attr()),
      ...
      //new DBField(FLD_dcsConfigString      , String.class        , DBField.TYPE_STRING(64)  , "DCS Configuration String"    , "edit=2"),
        ...
        new DBField(FLD_lastOilLevel         , Double.TYPE         , DBField.TYPE_DOUBLE      , "Last Oil Level"              , "format=#0.0"),
    ...
    public static final DBField NotificationFieldInfo[] = {
        ...
        new DBField(FLD_lastNotifyTime       , Long.TYPE           , DBField.TYPE_UINT32      , "Last Notify Time"            , _lastNotifyTime_attr()),
  ...
  //public static final String FLD_workOrderID           = "workOrderID";           // WorkOrder ID (may not be used)
    ...
    public static final DBField DataPushInfo[]           = {
        new DBField(FLD_lastDataPushTime     , Long.TYPE           , DBField.TYPE_UINT32      , "Last Data Push Time (sec)"   , "format=time"),
        new DBField(FLD_lastEventCreateMillis, Long.TYPE           , DBField.TYPE_INT64       , "Last Event Create Time (MS)" , "format=time"),
    };
            ...
            if (maxSpeedKPH > 0.0) { // must be greater-than 0.0
                ...
                if (evSpeedKPH > maxSpeedKPH) {
                    if (invalidSpeedAction == INVALID_SPEED_IGNORE) {
                        // ignore this record
                        Print.logWarn("Invalid EventData speed: "+evSpeedKPH+" [ignoring record per configuration]");
                        return false;
                    } else
                    if (invalidSpeedAction == INVALID_SPEED_TRUNCATE) {
                        // truncate speed to maximum
                        Print.logWarn("Invalid EventData speed: "+evSpeedKPH+" [set/truncate to "+maxSpeedKPH+"]");
                        evdb.setSpeedKPH(maxSpeedKPH);
                    } else {
                        // should not occur (just continue)
                        Print.logWarn("Invalid EventData speed: "+evSpeedKPH+" [unexpected action "+invalidSpeedAction+"]");
                    }
                }
            }
        }
    ...
    public static void main(String args[])
    {
        ...
        if (RTConfig.hasProperty(ARG_ZONECHECK)) {
            ...
            if (!deviceExists) {
                ...
                deviceRcd.setLastValidLocation(gp1);
